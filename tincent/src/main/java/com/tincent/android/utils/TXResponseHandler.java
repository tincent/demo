...
public class TXResponseHandler extends JsonHttpResponseHandler {
    ...
    public void onSuccess(ResponseHandlerInterface instance, int statusCode, Header[] headers, JSONObject response) {
        if (instance instanceof TXResponseHandler) {
            ...
            EventBus.getDefault().post(new TXNetworkEvent(handler.getRequestTag(), statusCode, "success", response));
        }
    }
    ...
    public void onFailure(ResponseHandlerInterface instance, int statusCode, Header[] headers, Throwable throwable, JSONObject errorResponse) {
        if (instance instanceof TXResponseHandler) {
            TXResponseHandler handler = (TXResponseHandler) instance;
            EventBus.getDefault().post(new TXNetworkEvent(handler.getRequestTag(), statusCode, "failure", errorResponse));
        }
    }
    ...
    public void onFailure(ResponseHandlerInterface instance, int statusCode, Header[] headers, String responseString, Throwable throwable) {
        if (instance instanceof TXResponseHandler) {
            TXResponseHandler handler = (TXResponseHandler) instance;
            EventBus.getDefault().post(new TXNetworkEvent(handler.getRequestTag(), statusCode, "failure", throwable));
        }
    }
